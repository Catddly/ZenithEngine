...
#	define ZE_CHECK_LOG(cond, ...) do { if (!(cond)) { ZE_LOG_FATAL(__VA_ARGS__); ZE_LOG_CRASH_POINT(); ZE_LOG_STACKTRACE(); ZE_FLUSH_LOG(); __debugbreak(); assert(false); } } while(false)
#else
...
#	define ZE_EXEC_CHECK_LOG(cond, ...) do { if (!(cond)) { ZE_LOG_FATAL(__VA_ARGS__); ZE_LOG_CRASH_POINT(); ZE_LOG_STACKTRACE(); ZE_FLUSH_LOG(); __debugbreak(); assert(false); } } while(false)
#else
